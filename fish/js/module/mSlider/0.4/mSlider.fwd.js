(function () {
    var mSlider = function (pParam, callback) {
        ...
        var param = {
            ...
            circle: false,
            canvas: ".mSlider_con ul",    //幻灯容器，与content配套使用，除了几个特定效果（如缩略图、123等）不需要外，其他都需要
            content: ".mSlider_con li",
            nav: ".mSlider_nav li",
            // 新增缩略图模式，如果需要，需指定showNav: "thumbNail"（观察需求）;新增自定义幻灯模式 showNav: "custom"
            showNav: "orange",
        ...
        scrollWH = oneWH * param.scrollNum;
         ...
         //排除用户自定义模式下在幻灯容器里添加以下样式
        if(param.showNav !=="custom"){
        ...
        if (param.autoScroll) {
            ...
            if(param.showNav ==="custom"){            //用户自定义模式下对幻灯主体内容和菜单停止和启动动画效果
                fish.all(param.canvas,this).hover(
                    stopAnim,
                    startAnim
                );
                fish.all(param.userNav,this).hover(
                    stopAnim,
                    startAnim
                );
            } else{
        ...
        if (param.showNav) {
            ...
            switch (param.showNav) {
                ...
                case "thumbnail":
                    ...
                    this.html("bottom", navHtml);
                    break;
                //添加自定义幻灯模式 showNav:"custom",同时添加一个用户自定义幻灯菜单属性userNav,传入幻灯切换的索引,如“#test ul li”
                case "custom":
                    param.nav = param.userNav;
                    break;
            }
            if(param.userNav){
                allNav = fish.all(param.nav);
            } else {
