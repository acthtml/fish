...
(function(){
    ...
    function main(param) {
        var beside, //表单内元素,后面被格式化为fish对象
            ...
            itemEnableFn = function(){}, //判定每汗是否有效的函数，无效的行不会加上 hover的类名。也就不会响应 mouseover和click等
        ...
        beside.on(
            ...
            function(e) {
                var that = fish.getTarget(e),
                    ...
                    tray = tableWrapElem,
                    handledUrl;
                if(fish.trim(that.value) !== that.value) {
                    that.value = fish.trim(that.value);
                }
                if(that.value === "" || that.value === param.placeholder) {
                    output.hide();
                    //空白或这等于holder时，不显示
                    return;
                }
                ...
                if(key === 13) {
                    ...
                    handledUrl = url && url.call(output, output.val());
                    if(handledUrl){
                        //异步调取数据
