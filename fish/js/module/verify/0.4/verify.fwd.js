    ...
    */
    ...
    var tool = {
        ...
        rule: {
            ...
            "isEnglish": /^[A-Za-z]+$/,
            "isChinese": /^[\u0391-\uFFE5]+$/,//包含中文标点
            "isChineseWord":/^[\u4e00-\u9fa5]+$/,//不包含中文标点
            "isEnOrCn": /^[A-Za-z\u0391-\uFFE5]+$/,
            "isEnOrCnWord": /^[A-Za-z\u4e00-\u9fa5]+$/,
            "isMail": /\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/,
        ...
        _fieldTypeToRegMapping: {
            ...
            eng: "isEnglish",
            chn: "isChinese",
            chnWord:"isChineseWord",
            enOrCn: "isEnOrCn",
            enOrCnWord:"isEnOrCnWord",
            mail: "isMail",
            url: "isUrl",
            currency: "isCurrency",
            number: "isNumber",
...
//        ,
//        focusFirstErrElem:function(){
//            //拿到所有的line元素（只包含一个input元素），找第一个有error的，有的话找那个input（也可能有多个。。。），变数很多。
//            var parentSelArr = this.lineSelecter.split(".");
//
//        }
    ...
    function getLocString(relativeElem, pos, gap) {
        ...
        var uRelElem = fish.one(relativeElem),
            ...
            left = relOffset.left,
            cssStr;
        if(uRelElem.attr("data-showType") == "static"){
            cssStr = "position:static;"
        }else{
