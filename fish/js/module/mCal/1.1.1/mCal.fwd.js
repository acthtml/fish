...
;(function () {
    ...
    */
    ...
    function main(param){
        ...
        var defParam = {
            css: "",
            tableThCss:"", //表格中th头的样式值
            userDateCss:"", //用户自定义数据div的样式值
            dateCss : "", //单元格中日期的样式值
            itemCss :"", //日历单元格div的样式值
            width:0,
            height:0,
            open: false,
            ...
            showPast: false,
            showDay : false,
            showMonthOnBtn : false,
            showSpecDate : true, //显示特殊日期文字
            now: "",
            focus:fish.parseTime(new Date()),  //当前日历的起一个月份
            eachHeight: 28, //单元格的高度包括边框
            eachWidth: 28, //单元格的宽度包括边框
            monthNum: 2, //显示几个月份
            backgroundMonthnumSize : 160, //月份背景数字的大小
            enableBtnWhenOutOfRange : true,
            ajaxUrl: "",
            ...
            prevBtnTemp:'<span class="lastMonthBg date_lastSpan"><span class="lastMonth"></span><span class="monthNum"></span></span>',
            nextBtnTemp:'<span class="nextMonthBg date_nextSpan"><span class="nextMonth"></span><span class="monthNum"></span></span>',
            cache:{},//缓存数据
    ...
    mCalFn = {
            ...
            if (this.inputElem && (this.id = fish.all(this.inputElem).attr("data-mcalid")) && cache[this.id] ) {
               ...
               //console.log("#242 the style is " + this.monthChangeStyle)
                ...
                for(var i=0; i<cMonthNum; i++){
                    (function(index){
                        that.month[i] = new mMonth({//缓存了几个月的面板
                            ...
                            cal : that,//日历对象
                            userDateCss : that.userDateCss,
                            itemCss : that.itemCss,
                            dateCss : that.dateCss,
                            tableThCss : that.tableThCss,
                            now : that.now,
                ...
                this.__prevBtn = fish.one(".lastMonthBg", this.__calElem).on("click", function(){
                    if(!that.__prevBtn.hasClass("clickDisable")){
                        this.beforePrevFn && this.beforePrevFn.call(that);
                        that.goPrev();
                    }
                });
                this.__prevBtnTips = fish.one(".monthNum", this.__prevBtn);
                this.__nextBtn = fish.one(".nextMonthBg", this.__calElem).on("click", function(){
                    if(!that.__nextBtn.hasClass("clickDisable")){
                        this.beforeNextFn && this.beforeNextFn.call(that);
                        that.goNext(); 
                    }
                });
                this.__nextBtnTips = fish.one(".monthNum", this.__nextBtn);
        ...
        setWH:function(){
            //获取本身的框高
            var newW = this.month[0].width * this.monthNum + (BORDERRIGHT_WIDTH-2)*(this.monthNum-1)-1,
                that = this;
//            this.height = this.__topElem.height() + 25 + 6*(this.eachHeight+2 +1);
            this.height = CAL_TOP_HEIGHT + 25 + 6*(this.eachHeight+2 +1);
        ...
        setNow:function(now){
            var nowTime = fish.parseTime(now);
            this.now = nowTime;
            fish.one(this.inputElem).val(nowTime);
            //这里咩有做显示和同步数据的操作
        },
        set:function(param, disableSetMonth){
            ...
            if(this.css){
                this.__monthHolderElem.css(this.css);
            }
...
//            }
        ...
        _scrollTo:function(group, position){
            ...
            this.monthGroup[lastGroup].forEach(function(monthObj, ii){
                monthObj.css(that._monthStyle((ii + px) * monthObj.width + (ii + px)*BORDERRIGHT_WIDTH));
                monthObj.z(z);
            });
        },
        //设置滚动
        ...
        _setStatic:function(){
            ...
            this.monthGroup.b.forEach(function(monthObj, ii){
                monthObj.css(that._monthStyle(ii * monthObj.width + ii*(BORDERRIGHT_WIDTH-1)));
            });
        },
        ...
        setMonthBtn:function(){
            var that = this,
                focus = that.focus,
                prevDate = fish.parseDate(focus, {months:-1}),
                prevMonth = prevDate.getMonth() + 1,
                startDate = fish.parseDate(that.startTime),
                endDate = fish.parseDate(that.endTime),
                prevEnable,
                nextDate = fish.parseDate(focus, {months:that.monthNum}),
                nextMonth = nextDate.getMonth() + 1,
                nextEnable;
            startDate.setDate(1);
            endDate.setDate(1);

            prevEnable = startDate <= prevDate;
            nextEnable = nextDate <= endDate;  
                          
            if(this.showMonthOnBtn){
                this.__prevBtnTips.html(prevMonth + "月").css("display:block");
                this.__nextBtnTips.html(nextMonth + "月").css("display:block");
            }
            else{
                this.__prevBtnTips.css("display:none");
                this.__nextBtnTips.css("display:none");
            }



            //clickDisable class
            if(!this.enableBtnWhenOutOfRange){
                prevEnable ? this.__prevBtn.removeClass("clickDisable") : this.__prevBtn.addClass("clickDisable");
                nextEnable ? this.__nextBtn.removeClass("clickDisable") : this.__nextBtn.addClass("clickDisable");
            }
            else{
                this.__prevBtnTips.removeClass("clickDisable");
                this.__nextBtnTips.removeClass("clickDisable");
            }
        },
            ...
            var defParam = {
                ...
                calTemp:(function(){
                    ...
                    dayString.split("").forEach(function(text){
                        temp.push("<th " + (text === "六" ? "class='sat'" : (text === "日" ? "class='sun'" : ""))  + " >" + text + "</th>");
                    });
                    temp.push('</tr>');
                    for(var a=0; a<6; a++){
    ...
    mMonthFn = {
        init:function(){
            ...
            if(!this.__elem){
                ...
                this.__thElems = fish.all("th", this.__elem);
                if(this.dateCss){
                    this.__dateElems.css(this.dateCss);
                }             
                if(this.userDateCss){
                    this.__userElems.css(this.userDateCss);
                }
                if(this.tableThCss){
                    this.__thElems.css(this.tableThCss);
                }
                if(this.itemCss){
                    this.__itemElems.css(this.itemCss);
                }
                ...
                //背景
                this.__bgElem = fish.all(".monthBg", this.__elem).css("font-size:" + this.cal.backgroundMonthnumSize + "px");
...
//                this.height = this.__elem.height();
        ...
        setDate:function(param){
            ...
            this.__itemElems.each(function(elem, ii){
                ...
                thisItem.removeClass("spanNow spanDate spanEnable spanHover spanSpec notAble");
                ...
                if(that.fillBlank||((startPrint || fish.one(elem).attr("calday") === firstDay) && !stopPrint)){
                    ...
                    if(specDate && that.cal.showSpecDate){
        ...
        setUserDate:function(){
            ...
            if(this.dataUrl && this.processDataFn && this.dateTempleteFn){
                ...
                dataUrl = dataUrl.replace(/{month}/g, getFormatedNumberStr(dateObj.getMonth()+1));
                dataUrl = dataUrl.replace(/{year}/g, dateObj.getFullYear());
                dataUrl = dataUrl.replace("{nextMonth}", nextMonthDate.getMonth()+1);
                dataUrl = dataUrl.replace("{nextMonthYear}", nextMonthDate.getFullYear());
                ...
                dataUrl = dataUrl.replace("{monthDays}",monthDays);//这个月有多少天
                this._clearUserDate();//清空之前用户打的缓存数据
                //是否需要重新加载数据
                if(this.reloadData || !this.cal.cache[keyUId]||!this.cachAjax){
        ...
        _getMonData:function(date){
            var dataUrl = this.dataUrl,
                ...
                nextMonthDate = fish.parseDate(date,{months:1}),
                that = this;
            if(ajaxObj){
                try{
                    ajaxObj.abort();
            ...
            dataUrl = dataUrl.replace(/month/g, month);
            dataUrl = dataUrl.replace(/year/g, year);
            dataUrl = dataUrl.replace("{nextMonth}", nextMonthDate.getMonth()+1);
            dataUrl = dataUrl.replace("{nextMonthYear}", nextMonthDate.getFullYear());
            dataUrl = dataUrl.replace("{monthDays}",monthDays);//这个月有多少天
            //遍历
        ...
        _setUserDate:function(type){
            ...
            this.__userElems.each(function(elem, ii){
                ...
                if(!stopPrint && (startPrint ||
                    ...
                    startPrint = true;
                    printTime = fish.parseTime([year, month, printDate].join("-"));
                    specDate = getSpecDate(printTime);
                    enableDate = printTime >= that.startTime && printTime <= that.endTime;
                    ...
                    userCont = that.dateTempleteFn(year, month, printDate, data[printDate-1], specDate, enableDate);
                    if(that.dateClickAbleFn){
                        dateClickEnable = that.dateClickAbleFn(year, month, printDate, data[printDate-1], specDate);
                    }
