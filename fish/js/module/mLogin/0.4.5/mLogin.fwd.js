...
;(function(){
    ...
    var eventsBinded = {
    		login				:	false,
    		passWord			:	false,
    		passSending			:	false,	//找回密码的倒计时
    		passSubmit			:	false,	//找回密码的提交密码
    		resetPass			:	false,	//重置密码
    		passValida			:	false,	//忘记密码短信验证
    		restPassWord		:	false,	//设置新密码
    		repeatNewPassword	:	false	//新的密码
    }
    ...
    function setTips(obj, text) {
        if (obj && text) {
            obj.innerHTML = text;
        	fish.one(obj).removeClass("none");
        }
    }
    function hideTips(obj) {
        if (obj) {
            obj.innerHTML = "";
        	fish.one(obj).addClass("none");
        }
    }
    ...
    function logObj(param) {
        var nfn = function (){},
            ...
            thisParam = {};
        fish.lang.extend(thisParam, defaultParam);
        fish.lang.extend(thisParam, param);
        fish.lang.extend(this, thisParam);
        this.init();
    }
...
//            }
    ...
    function mLogin(param, callback){
        ...
        if(window.location.host.indexOf(".17u.cn") > -1){
            window.document.domain="17u.cn";
        }
    	//干掉以前的showSignin(因为in表示登录)，换成showSignup，表示显示登录弹框的注册入口
    	if(param.showSignin){
    		param.showSignup = param.showSignin;
    	}
        var nfn = function(){},
            def = {
        		showing			:		false,	//判断是否显示 
                title           :       "",     //弹出框的标题
                show            :       false,  //是否立即显示登录组件
                showSignupFirst	:		false,	//是否第一个显示注册弹框
                showSignup		:		false,	//显示登录弹框上面的注册入口
                showBooking     :       false,  //是否显示直接预定
//                showSignin      :       false,  //是否显示免费注册（老的，现在换成showSignup）
                showPartner     :       false,  //是否显示合作登录
                enableUsername  :       true,   //是否允许更改用户名
                username        :       "",     //默认的用户名
                loginUrl        :       "/Member/MemberLoginAjax.aspx",                                 //登录接口地址
                signupUrl		:		"/Member/MemberLoginAjax.aspx",
                getPassUrl      :       "http://passport.17u.cn/member/GetPasswordAjaxCall.aspx?Type=GetCodeByMoblie",               //获取验证码url
                verifyPassUrl   :       "http://passport.17u.cn/member/GetPasswordAjaxCall.aspx?Type=ValidCodeByMoblie",             //验证url
                resetPassUrl    :       "http://passport.17u.cn/member/GetPasswordAjaxCall.aspx?Type=ChangePassword&typeid=1",       //重置密码的url
        ...
        fish.lang.extend(thisParam, def);
        fish.lang.extend(thisParam, param);
        fish.lang.extend(the, thisParam);

        the.init();

        callback && callback(the);
//        mLoginArry[mLoginArry.length] = the;
        mLoginArry.push(the);
//        console.log(mLoginArry);
        return the;
    }
    mLogin.onSignUpSuccess = function(){
    	for(var n = 0 ; n < mLoginArry.length; n++){
    		if(mLoginArry[n].showing){
    			mLoginArry[n].onLogin && mLoginArry[n].onLogin();
    		}
    	}
    }
    mLogin.onSwitchToLogin = function(){
    	for(var n = 0 ; n < mLoginArry.length; n++){
    		if(mLoginArry[n].showing){
    			mLoginArry[n].open && mLoginArry[n].open();
    		}
    	}
    }
    ...
    mLoginFn = {
                           ...
                           <form action="" id="login_form_pop_mLogin">\
	                           <div class="login_content">\
	                                   ...
	                                   <div class="login_tips clearfix">\
	                                       <span class="none tipsText" id="titleTips_pop_mLogin">用户名或密码错误</span>\
	                                   </div>\
	                                   <div class="mLogin_list clearfix">\
                                           <label for="account_pop_mLogin" class="mLogin_left">\
                                               账号：</label>\
	                                       <span class="mLogin_right">\
	                                           <input type="text" placeholder="手机/邮箱" name="name" maxlength="30" id="account_pop_mLogin" placeholderoption="手机/邮箱" >\
	                                       </span>\
	                                   </div>\
	                                   <div class="mLogin_list clearfix">\
                                           <label for="actpwd_pop_mLogin" class="mLogin_left">\
                                               密码：</label>\
	                                       <span class="mLogin_right">\
	                                           <input type="password" name="pass" maxlength="30" id="actpwd_pop_mLogin">\
	                                       </span>\
	                                   </div>\
	                                   <div class="none mLogin_remember mLogin_list">\
                                           <input type="checkbox" checked="" id="rem_it_1w_pop_mLogin" class="ceck">\
                                           <label class="ri1_label" for="rem_it_1w_pop_mLogin">\
                                               记住密码（一周）</label>\
	                                   </div>\
	                                   <div class="mLogin_signon clearfix">\
			                               <span>\
				                               ...
				                               <a class="forget_password" href="http://Passport.17u.cn/Passport/GetPassword/" target="_blank">\
				                                   忘记密码？</a>\
				                           </span>\
	                                   </div>\
	                               </div>\
	                               <div class="clearfix">\
	                                   <span class="partner none">\
	                                   ...
	                                   <span class="signin none">\
	                                       <div class="boxtr clearfix">\
	                                            <span class="l2_text">还不是同程会员？\
	                                            </span><a class="free_signup" target="_blank" href="http://passport.17u.cn/register/index/" title="注册">免费注册</a>\
	                                       </div>\
	                                   </span>\
	                               </div>\
	                           </div>\
            ...
            fish.one(this.holder).html("bottom",
                '<div id="mLogin_pass" class="mLogin_content submit_pass clearfix">\
                    ...
                    <p class="btn_line clearfix">\
                        <a class="submit_btn btn_outer clearfix" href="javascript:;">\
                            <span class="btn_inner">提&nbsp;交</span></a>\
                        <span style="display: none;" class="wait4sign_a submit_btn">\
                        <img alt="" src="http://img1.40017.cn/cn/new_ui/public/images/wait.gif"></span>\
                        <span class="btn_tips"></span>\
                    </p>\
                    <p class="tips_line">\
                        如果您的手机已停机或其他原因无法获取验证码，请致电 4007-777-777 转3 人工服务\
                    </p>\
                </div>');
        },
                ...
                <table cellspacing="0" cellpadding="0" border="0" class="pass_content">\
                               <tbody>\
                                   ...
                                   <tr class="btn_rol">\
                                       ...
                                       <td colspan="2">\
                                           <a class="pass_btn btn_outer clearfix" href="javascript:;"><span class="btn_inner">完&nbsp;成</span></a>\
        ...
        bulidSingup : function(){
        	fish.one(this.holder).html("bottom",
        			'<div id="freeSignup" class="free_signupBox clearfix">\
                    <div class="free_title">快速注册</div>\
                    <div class="free_content clearfix">\
                        <div class="top_login">\
                           <span class="btn_close"></span>\
                        </div>\
                        <div class="free_info">\
                        	<div class="free_infoBox">\
	                        	<iframe frameborder="0" src="http://passport.17u.cn/register/MemberRegister/" class="port_signUp">\
                        		</iframe>\
                        	</div>\
                        	<div class="clearfix free_haveLogin">\
                        		我已注册，<a href="http://passport.17u.cn" class="downOpenLogin">立即登录</a>\
                        	</div>\
                        </div>\
                    </div>\
    			</div>');
        ...
        init : function(){
            ...
            if(!fish.dom("#mLogin_holder")){
                fish.one(document.body).html("bottom", "<div id='mLogin_holder' style='position:absolute;left:-1000px;top:0px'></div>");
            }
            ...
            this.__loginContentElem = fish.one(".mLogin_content", this.loginElem);
            //提交验证码框
            ...
            //注册弹框
            if(!fish.dom("#freeSignup")){
            	this.bulidSingup();
            }
            this.freeSignupElem = fish.dom("#freeSignup");


            if(!this.enableUsername){
                fish.one("#account_pop_mLogin").attr("disabled", "disabled");
            }else{
            	fish.dom("#account_pop_mLogin").disabled = null;
            }

            if(this.username){
                fish.one("#account_pop_mLogin").val(this.username);
            }else{
                fish.one("#account_pop_mLogin").val("");
            }

            if(this.title){
                fish.all(".mLogin_bg", this.loginElem).html(this.title);
            }else{
                fish.all(".mLogin_bg", this.loginElem).html("");
            }
            if(this.showPartner){
                ...
                fish.all(".partner", this.loginElem).addClass("none");
            }

            if(this.showSignup){
                fish.all(".signin", this.loginElem).removeClass("none");
            }
            else{
                fish.all(".signin", this.loginElem).addClass("none");
            }

            //读取并设置高度
            //fish.one(this.holder).css("display:block");
//            fish.one( that.loginElem).css("height:" + that.__loginContentElem.height() + "px");
            //fish.one(this.holder).css("display:none");
            ...
            fish.one(".btn_close", this.freeSignupElem).on("click", closeBox)

            this.resetBookingBoxState && this.resetBookingBoxState();
            //登录组件
            ...
            // 注册事件
            if(this.freeSignupElem){
            	//立即登录
            	fish.dom("a.downOpenLogin",this.freeSignupElem).onclick = function(e){fish.preventDefault(e);that.open(true)};
            }
            
            //绑定登录弹框事件
            if(!eventsBinded.login){
            	fish.all("#mLogin_login .forget_password").on("click", function(e){
            		fish.preventDefault(e);
            		if (fish.trim(that.loginObj.nameInput.value).length === 0 || fish.trim(that.loginObj.nameInput.value) == "手机/邮箱" || !fish.valida.phone(fish.trim(that.loginObj.nameInput.value))) {
            			setTips(that.loginObj.tips, "请输入您的手机号");
            		}else{
            			that.getPass();
            			that.showPassBox();
            	...
            	//登录弹出注册弹框
            	fish.all("#mLogin_login .free_signup").on("click", function(e){
            		fish.preventDefault(e);
            		fish.mPop.close();
            		hideTips("#freeTips");
            		fish.mPop({
            			content : fish.one("#freeSignup"),
            			beforeOpen : function(){that.showing = true},
            			afterClose:function(){that.showing = false}
            		})
            	});
            	//是否绑定事件置为true
            	eventsBinded.login = true;
            }
            //注册组件
            
            //倒计时按钮文字的节点
            this.__timeoutTextElem = fish.one(".sending_btn .btn_a_inner", this.passElem);
            this.__teleTextElem = fish.one(".tips_line", this.passElem);
            this.__passTipsElem = fish.one(".btn_tips", this.passElem);
            this.__usernameElem  = fish.one("#account_pop_mLogin");
            this.__titleElem = fish.one("div.mLogin_bg",this.loginElem);


            if(this.telephoneTips){
                this.__teleTextElem.html(this.telephoneTips);
            }

            //倒计时按钮节点
            this.__timeoutBtnElem = fish.one(".sending_btn", this.passElem);
        	if(!eventsBinded.passSending){
        		this.__timeoutBtnElem.on("click", function(){
        			if(!that.__timeoutBtnElem.hasClass("btn_disable")){
        				that.getPass();
        			}
        		});
        		eventsBinded.passSending = true;
        	}
            //验证按钮
            this.__verifyPassBtnElem = fish.one(".submit_btn", this.passElem);
            if(!eventsBinded.passSubmit){
            	this.__verifyPassBtnElem.on("click", function(){
            		that.verifyPass();
            	});
            	eventsBinded.passSubmit = true;
            }
            //重置按钮
            this.__resetPassBtnElem = fish.one(".pass_btn", this.resetPassElem);
            if(!eventsBinded.resetPass){
            	this.__resetPassBtnElem.on("click", function(){
            		that.resetPass();
            	});
            	eventsBinded.resetPass = true;
            }
            ...
            if(!eventsBinded.passValida){
            	this.__passInputElem.on("click", function(){
            		this.select();
            	})
            	.on("focus", function(){
            		that._hidePassErr();
            	...
            	eventsBinded.passValida = true;
            }

            this.__newPassInputElem = fish.all("#mLogin_resetpassword");
            if(!eventsBinded.restPassWord){
            	this.__newPassInputElem.on("click", function(){
            		this.select();
            	})
            	.on("focus", function(){
            		that._hideNewPassErr();
            	...
            	eventsBinded.restPassWord = true;
            }

            this.__repeatPassInputElem = fish.all("#mLogin_repeatpassword");
            if(!eventsBinded.repeatNewPassword){
            	this.__repeatPassInputElem.on("click", function(){
            		this.select();
            	})
            	.on("focus", function(){
            		that._hideRepeatPassErr();
            	...
            	eventsBinded.repeatNewPassword = true;
            }

            if(this.show){
                this.open(true);
            }
            if(this.showSignupFirst){
            	this.openFreeSignup();
            	this.showSignupFirst = false;
            }
        },
        
        resetBookingBoxState:function(){
        	if(this.showBooking){
        		var that = this;
                fish.all(that.loginElem).css("width:600px");
                fish.all(".l_border", this.loginElem).css("display:block");
                fish.dom(".yd_bton", this.loginElem).onclick = function (){
                    that.onBooking();
                    fish.mPop.close();
                    that.onClose();
                }
        ...
        open:function(callFromInit){
            this.onOpen && this.onOpen();
            this.resetBookingBoxState && this.resetBookingBoxState();


            hideTips(this.loginObj.tips);
            var that = this;
            if(!callFromInit){//防止循环调用
                this.init();
            }
            if(this.showSignupFirst){
            	fish.mPop({
            		content:this.freeSignupElem,
            		beforeOpen : function(){that.showing = true},
            		afterClose:function(){that.showing = false}
        		});
            }else{
            	fish.mPop({
            		content:this.loginElem,
            		beforeOpen : function(){that.showing = true},
            		afterClose:function(){that.showing = false}
        		});
            }
        ...
        setTitle : function(newtitle){
            this.__titleElem.html(newtitle);
        },
        //获取验证码
        getPass : function(){
            var that = this;
...
//                        }
        ...
        _verifyPassCallBack : function(data){
            var dataStatus = data.state;
            this.onGetPassAjaxBack && this.onGetPassAjaxBack(dataStatus);
        ...
        _getPassCallBack : function(data){
            ...
            if(dataStatus && dataStatus == 1){
               ...
               if (dataStatus == "007"){
                ...
                else if (dataStatus == "049"||dataStatus == "3") {
                    this._showVerifyPassTips("对不起，您本月申请修改密码次数已达上限，如您仍需修改密码，请致电4007-777-777。");
                }
                else if (dataStatus == "010" || dataStatus == "003"){
                    this._showVerifyPassTips("对不起，您的手机号还未注册，立即<a target='_blank' href='http://www.17u.cn/reg.aspx'>注册</a>");
                }
                else if (dataStatus == "050") {
                    this._showVerifyPassTips("对不起，您今天申请修改密码次数已达上限，如您仍需修改密码，请致电4007-777-777。");
                }
                else if (dataStatus == "4") {
                    this._showVerifyPassTips("您已经超过每日获取短信上限，请次日再试。");
                }
                else if (dataStatus == "5") {
                    this._showVerifyPassTips("手机号码与账号手机号码不同。");
                }
                else if (dataStatus == "6") {
                    this._showVerifyPassTips("60s内只能获取一次短信，请稍后再试。");
                }
                else{
        ...
        _resetPassCallBack : function(data){
            var dataStatus = data.state;
            this.onResetPassAjaxBack && this.onResetPassAjaxBack(dataStatus);
            if (dataStatus && dataStatus == 1) {
                this._resetPassSuc();
            }
            else if(dataStatus){
